...
                                                <input
                                                    type="checkbox"
                                                    checked={!!selectedQuestionsForImport[question.id]} // Changed this line
...
const duplicateImageInStorage = async (sourceUrl) => {
    try {
        ...
        showSnackbar(`Failed to duplicate image: ${e.message}`, 'error');
        return null;
    }
};
...
        : Timestamp.now().toMillis(); // If no questions, start from now
    ...
    const importedQuestionsWithNewImages = await Promise.all(
        questionsToImport.map(async (q) => {
            ...
            // Duplicate question image if it exists
            if (newQuestionData.imageUrl) {
                newQuestionData.imageUrl = await duplicateImageInStorage(newQuestionData.imageUrl);
            }

            // Duplicate option images if they exist
            newQuestionData.options = await Promise.all(
                newQuestionData.options.map(async (option) => {
                    const newOptionData = { ...option };
                    if (newOptionData.imageUrl) {
                        newOptionData.imageUrl = await duplicateImageInStorage(newOptionData.imageUrl);
                    }
...
    importedQuestionsWithNewImages.forEach((q, index) => {
...
      <main className="flex-1 p-8 bg-gray-50 flex flex-col items-center pt-20">
