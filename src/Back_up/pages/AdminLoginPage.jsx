...
const AdminLoginPage = () => {
  ...
  return (
    <> {/* NEW React Fragment to allow multiple top-level elements */}
      {/* NEW HEADER */}
      ...
      <div className="flex flex-col items-center justify-center min-h-screen p-4 sm:p-6 bg-white">
        ...
        <div className="bg-white p-8 sm:p-10 rounded-2xl shadow-xl border border-gray-200 w-full max-w-md transform transition-all duration-300 hover:scale-[1.01] hover:shadow-2xl">
          {/* Title - CHANGED TO RED TEXT (removed gradient) */}
          ...
          <form onSubmit={handleLogin} className="flex flex-col space-y-6">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  type="email"
                  id="email"
                  className="pl-10 pr-4 py-3 border border-red-300 rounded-lg w-full text-gray-800 text-base leading-tight bg-white transition duration-200 placeholder-gray-400 focus:outline-none focus:border-red-500 focus:ring-2 focus:ring-red-500" // Border and focus colors changed to red
                  placeholder="Email"
                  value={email}
            ...
            <div>
              ...
              <div className="relative">
                {/* Lock Icon color changed to red-400 */}
                <div className="absolute left-3 top-1/2 -translate-y-1/2 text-red-400 h-5 w-5 flex items-center pointer-events-none">
                  <Lock size={20} />
                </div>
                <input
                  ...
                  id="password"
                  className="pl-10 pr-10 py-3 border border-red-300 rounded-lg w-full text-gray-800 text-base leading-tight bg-white transition duration-200 placeholder-gray-400 focus:outline-none focus:border-red-500 focus:ring-2 focus:ring-red-500" // Border and focus colors changed to red
                  placeholder="********"
                  value={password}
                  onChange={(e) => setPassword(e.target.value)}
                  required
                />
                {/* Toggle Button for Password Visibility - Icon color changed to red-500 */}
                <button
                  type="button"
                  onClick={() => setShowPassword(!showPassword)}
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-red-500 hover:text-red-700 focus:outline-none" // Icon color changed to red
                >
                  {showPassword ? <EyeOff size={20} /> : <Eye size={20} />}
                </button>
              </div>
            </div>
            {/* Login Button - Changed to red gradient */}
            <button
              type="submit"
              className="w-full bg-gradient-to-r from-red-600 to-red-800 text-white font-bold py-3 px-6 rounded-lg shadow-lg transform transition-all duration-300 hover:scale-105 hover:from-red-700 hover:to-red-900 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none disabled:shadow-none focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 focus:ring-offset-white" // Gradient changed to red tones
              disabled={loading}
            >
              {loading ? (
