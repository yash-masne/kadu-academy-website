...
import { getFirestore, collection, query, where, onSnapshot, doc, updateDoc, deleteDoc, addDoc, getDocs, Timestamp, getDoc, writeBatch } from 'firebase/firestore';
...
const duplicateImageAndGetUrl = async (sourceUrl) => {
    ...
    try {
      ...
      const storageRef = ref(getStorage(), `question_images/${newFileName}`);
...
const AdminTestListPage = () => {
  ...
  const _duplicateTest = (originalTestId, originalTestTitle, originalTestData) => {
    showConfirm(
        ...
        async () => {
          setShowConfirmModal(false);
          showSnackbar(`Duplicating test "${originalTestTitle}"...`, 'info');

          try {
              // Step 1: Create the new test document with initial data
              const newTestData = {
                  ...
                  isQuestionBank: originalTestData.isQuestionBank ?? false, // Add this line
              ...
              // Step 2: Fetch all questions from the original test
              const originalQuestionsSnapshot = await getDocs(query(collection(getFirestore(), 'tests', originalTestId, 'questions'), orderBy('order', 'asc')));
              
              // Step 3: Duplicate each question and its images
              const newQuestionsCollectionRef = collection(newTestDocRef, 'questions');
              const questionsToDuplicate = originalQuestionsSnapshot.docs.map(qDoc => qDoc.data());

              const duplicatedQuestionsPromises = questionsToDuplicate.map(async (questionData) => {
                  const newQuestionData = { ...questionData, createdAt: Timestamp.now(), updatedAt: Timestamp.now() };

                  // Duplicate the main question image if it exists
                  if (newQuestionData.imageUrl) {
                      newQuestionData.imageUrl = await duplicateImageAndGetUrl(newQuestionData.imageUrl);
                  }

                  // Duplicate images for each option
                  if (newQuestionData.options && newQuestionData.options.length > 0) {
                      newQuestionData.options = await Promise.all(
                          newQuestionData.options.map(async (option) => {
                              if (option.imageUrl) {
                                  return {
                                      ...option,
                                      imageUrl: await duplicateImageAndGetUrl(option.imageUrl),
                                  };
                              }
                              return option;
                          })
              ...
              // Wait for all questions and their images to be duplicated
              const newQuestionsData = await Promise.all(duplicatedQuestionsPromises);
              
              // Step 4: Add the newly prepared questions to the new test
              const batch = writeBatch(getFirestore());
              newQuestionsData.forEach((questionData) => {
                  const newQuestionRef = doc(newQuestionsCollectionRef);
                  batch.set(newQuestionRef, questionData);
              });
              await batch.commit();

              // Step 5: Update the totalQuestions count on the new test document
              const newTotalQuestions = newQuestionsData.length;
              await updateDoc(newTestDocRef, { totalQuestions: newTotalQuestions, updatedAt: Timestamp.now() });
  ...
  try {
    ...
    // Then, call the Cloud Function for notifications
    if (scheduledTime) {
      try {
        const functions = getFunctions(getApp());
        const sendScheduledTestNotification = httpsCallable(functions, 'sendScheduledTestNotification');

        const result = await sendScheduledTestNotification({
          testId: testId,
          testTitle: testTitle,
        ...
        if (result.data.success) {
          ...
          throw new Error(result.data.message || 'Failed to call Cloud Function');
        }
        
      } catch (e) {
        showSnackbar(`Failed to send scheduled notification: ${e.message}`, 'error');
        console.error('Callable function error:', e);
      }
    } else {
      showSnackbar(`Test "${testTitle}" scheduled successfully!`, 'success');
    }
          ...
          {loading ? (
            ...
            <div className="space-y-6">
              {tests.map((test) => {
                ...
                const isQuestionBankTest = test.isQuestionBank ?? false;
...
return (
                  <div key={test.id} className="bg-white p-6 rounded-xl shadow-lg border border-gray-200 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                    <div className="flex-1">
                      ...
                      {/* NEW: Hide test type and other details for question banks */}
                      {!isQuestionBankTest && (
                        <>
                    ...
                    <div className="flex flex-wrap items-center justify-end gap-2 mt-4 lg:mt-0">
                      {/* The Duplicate and Edit buttons remain visible for all test types */}
                      <button
                      ...
                      {/* All other buttons are hidden for question banks */}
                      {!isQuestionBankTest && (
                        <>
                          ...
                          {isPublished ? (
                            <button
                              ...
                              className={`flex items-center gap-1 py-2 px-3 text-sm rounded-lg text-white font-bold transition-colors bg-red-600 hover:bg-red-700`}
                            >
                              <PublishIcon />
                              Unpublish
                            </button>
                            ...
                            <>
                              <button
                                ...
                                className={`flex items-center gap-1 py-2 px-3 text-sm rounded-lg text-white font-bold transition-colors bg-green-600 hover:bg-green-700`}
                              >
                                <PublishIcon />
                                Publish
                              </button>
                              <button
                                ...
                                className={`flex items-center gap-1 py-2 px-3 text-sm rounded-lg text-white font-bold transition-colors bg-blue-600 hover:bg-blue-700`}
                              >
                                <ScheduledIcon />
                                Schedule
                              </button>
