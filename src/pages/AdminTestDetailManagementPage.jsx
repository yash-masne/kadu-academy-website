...
import { getFirestore, collection, query, orderBy, onSnapshot, doc, updateDoc, addDoc, deleteDoc, getDoc, Timestamp, writeBatch, getDocs, where } from 'firebase/firestore';
...
    const [editingQuestionId, setEditingQuestionId] = useState(null);
    ...
    const [filterSection, setFilterSection] = useState('All'); // NEW: State for section filter
...
useEffect(() => {
    if (filterSection === 'All') {
        setFilteredQuestions(questionsFromSelectedTest);
    } else {
        setFilteredQuestions(questionsFromSelectedTest.filter(q => q.sectionName === filterSection));
    }
}, [questionsFromSelectedTest, filterSection]);
...
                                <div className="flex justify-between items-center mb-3">
    <h4 className="text-lg font-bold text-gray-800">Questions from "{selectedTestToImportFrom.title}"</h4>
    <div className="flex items-center gap-2">
        <span className="text-sm text-gray-600">Filter Section by:</span>
        <select
            value={filterSection}
            onChange={(e) => setFilterSection(e.target.value)}
            className="px-3 py-1 text-sm rounded-md border border-gray-300 focus:ring-2 focus:ring-red-500"
        >
            <option value="All">All ({questionsFromSelectedTest.length})</option>
            {selectedTestToImportFrom.sections?.map((section, index) => {
                const sectionCount = questionsFromSelectedTest.filter(q => q.sectionName === section.name).length;
                return <option key={index} value={section.name}>{section.name} ({sectionCount})</option>
            })}
        </select>
    </div>
</div>
...
                                                   <p className="text-sm font-medium text-gray-800 break-words mb-2" style={{ whiteSpace: 'pre-wrap' }}>
    <span className="font-bold text-base mr-2">{index + 1}.</span> {/* NEW: Display the question number */}
    {/* {question.isImageAboveQuestion && question.imageUrl && <img src={question.imageUrl} alt="Question" className="max-h-16 mb-2 object-contain" />} */}
...
const formatTextWithBold = (text) => {
    ...
    // If the number of parts is even, it means there's an unmatched **.
    // In this case, we return the string with the ** intact and no bolding.
    if (parts.length % 2 === 0) {
        return text;
    }
    ...
    let result = '';
    for (let i = 0; i < parts.length; i++) {
        // Every other part (at odd indices) is the text to be bolded.
        if (i % 2 !== 0) {
            result += `<strong>${parts[i]}</strong>`;
        } else {
            result += parts[i];
...
// ... (inside AdminTestDetailManagementPage component)

const addSection = async (sectionName) => {
  const newSection = { name: sectionName.trim() };
  const updatedSections = [...testSections, newSection];
  
  try {
    // Optimistically update the local state first for a responsive UI
    setTestSections(updatedSections);

    // Update the Firestore document for the test
    await updateDoc(doc(db, 'tests', testId), {
      sections: updatedSections,
      updatedAt: Timestamp.now()
    });
    
    showSnackbar(`Section '${sectionName}' added successfully!`, 'success');
  } catch (e) {
    // If the database update fails, revert the local state and show an error
    setTestSections(testSections);
    showSnackbar(`Failed to add section: ${e.message}`, 'error');
    console.error("Error adding section:", e);
  }
};

// NEW: Helper functions to manage the rename UI
const startRenameSection = (oldName) => {
...
// NEW: Function to handle renaming a section
const renameSection = async (oldName, newName) => {
  ...
  showSnackbar(`Renaming section from "${oldName}" to "${newName}"...`, 'info');

  const originalSections = [...testSections]; // Save original state for rollback
  const updatedSections = testSections.map(s => s.name === oldName ? { ...s, name: newName } : s);
  setTestSections(updatedSections); // Optimistically update the UI
  ...
  try {
    ...
    batch.update(testRef, { sections: updatedSections });

    const q = query(questionsCollectionRef, where('sectionName', '==', oldName));
    const querySnapshot = await getDocs(q);
    querySnapshot.docs.forEach(docSnap => {
      const questionRef = doc(questionsCollectionRef, docSnap.id);
      batch.update(questionRef, { sectionName: newName });
    });

    await batch.commit();
    showSnackbar(`Section "${oldName}" renamed to "${newName}" successfully!`, 'success');
    cancelRenameSection();
  } catch (e) {
    setTestSections(originalSections); // Revert the state on error
    showSnackbar(`Failed to rename section: ${e.message}`, 'error');
    console.error('Error renaming section:', e);
  }
};


// NEW: Function to handle deleting a section
const deleteSection = async (sectionName) => {
  showConfirm(
    'Confirm Deletion',
    `Are you sure you want to delete the section "${sectionName}"? All questions in this section will be unassigned.`,
    async () => {
      setShowConfirmModal(false);
      showSnackbar(`Deleting section "${sectionName}"...`, 'info');
      ...
      try {
        ...
        const updatedSections = testSections.filter(s => s.name !== sectionName);
        batch.update(testRef, { sections: updatedSections });

        const q = query(questionsCollectionRef, where('sectionName', '==', sectionName));
        const querySnapshot = await getDocs(q);
        querySnapshot.docs.forEach(docSnap => {
          const questionRef = doc(questionsCollectionRef, docSnap.id);
          batch.update(questionRef, { sectionName: '' });
        });

        await batch.commit();
        showSnackbar(`Section "${sectionName}" deleted successfully!`, 'success');
      } catch (e) {
        showSnackbar(`Failed to delete section: ${e.message}`, 'error');
...
const [parentQuestionForInsert, setParentQuestionForInsert] = useState(null);
...
useEffect(() => {
    const fetchTestData = async () => {
        ...
        try {
            ...
            if (testDocSnap.exists()) {
                const fetchedData = testDocSnap.data();
                // Set all test details states from the fetched data
                setTitle(fetchedData.title ?? '');
                setDescription(fetchedData.description ?? '');
                setDuration(fetchedData.durationMinutes?.toString() ?? '');
                setMarksPerQuestion(fetchedData.marksPerQuestion?.toString() ?? '1.0');
                setIsNegativeMarking(fetchedData.isNegativeMarking ?? false);
                setNegativeMarksValue(fetchedData.negativeMarksValue?.toString() ?? '0.0');
                setEnableOptionE(fetchedData.enableOptionE ?? true);

                // Set the test type and related multi-selects
                if (fetchedData.isFree) {
                    setSelectedTestType('Free');
                } else if (fetchedData.isPaidCollege) {
                    setSelectedTestType('College Student');
                    setSelectedCollegeBranches(fetchedData.allowedBranches ?? []);
                    setSelectedCollegeYears(fetchedData.allowedYears ?? []);
                } else if (fetchedData.isPaidKaduAcademy) {
                    setSelectedTestType('Kadu Academy Student');
                    setSelectedKaduCourses(fetchedData.allowedCourses ?? []);
                }

                // *** This is the key line for the fix ***
                setTestSections(fetchedData.sections ?? []);
            } else {
                showSnackbar('Test not found. Redirecting.', 'error');
                navigate('/admin-test-list');
            }
        } catch (err) {
            showSnackbar('Failed to load test data. Redirecting.', 'error');
    ...
    // Authentication logic remains the same, but separated
...
useEffect(() => {
  if (filterSection === 'All') {
    ...
    setFilteredQuestions(questions.filter(q => q.sectionName === filterSection));
  }
...
const deleteQuestion = (questionId, questionText) => {
  // NEW: Add a default value to questionText to prevent it from being undefined.
  const textToDisplay = questionText || 'Question with no text';
  const truncatedText = textToDisplay.length > 150 ? `${textToDisplay.substring(0, 150)}...` : textToDisplay;
  ...
  showConfirm(
    ...
    async () => {
      ...
      try {
        const questionsCollectionRef = collection(db, 'tests', testId, 'questions');
        const querySnapshot = await getDocs(query(questionsCollectionRef, orderBy('order', 'asc')));
        const currentQuestions = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
...
setQuestionSection(''); // NEW: Reset the section
...
const addOrUpdateQuestion = async () => {
  ...
  if (questionTextPart1.trim() === '' && !questionImageUrl) {
    showSnackbar('Question must have text or an image.', 'error');
    return;
  }
  // if (questionSection === '') {
  //   showSnackbar('Please select a section for the question.', 'error');
  //   return;
  // }
  ...
  try {
   ...
   const questionData = {
      questionText: isImageInBetween ? `${questionTextPart1} ${questionTextPart2}` : questionTextPart1,
...
      // NEW: Add the sectionName to the question document
...
    };
    ...
    // NEW: Update the main test document with the new list of sections
    await updateDoc(doc(db, 'tests', testId), {
...
const clearQuestionForm = () => {
    ...
    // NEW: Reset the parentQuestionForInsert state
    setParentQuestionForInsert(null);
};

// ... (inside the AdminTestDetailManagementPage component)
// ... (after the addOrUpdateQuestion function)

// NEW: Function to set up the form for adding a question below
const handleAddQuestionBelow = (parentQuestion) => {
  // Clear the form to start fresh
  clearQuestionForm();
  
  // Set the state for the parent question and inherit the section
  setParentQuestionForInsert(parentQuestion);
  setQuestionSection(parentQuestion.sectionName ?? '');
...
// NEW: Function to save the new question with correct ordering
const saveQuestionBelow = async () => {
    // Basic validation
    if (!parentQuestionForInsert) {
        showSnackbar('Error: No parent question selected for insertion.', 'error');
        return;
    }
    if (questionTextPart1.trim() === '' && !questionImageUrl) {
      showSnackbar('Question must have text or an image.', 'error');
    ...
    showSnackbar('Adding question...', 'info');
    
    try {
        const questionsCollection = collection(db, 'tests', testId, 'questions');
        const batch = writeBatch(db);
        
        // Get the order of the parent question
        const parentOrder = parentQuestionForInsert.order;
        
        // Increment the order of all questions that come after the parent
        const questionsToReorder = questions.filter(q => q.order > parentOrder);
        questionsToReorder.forEach(q => {
            const questionRef = doc(questionsCollection, q.id);
            batch.update(questionRef, { order: q.order + 1 });
        });
        ...
        // Add the new question to the batch
        ...
        // Commit the batch to save all changes at once
        await batch.commit();

        // Update total question count on the main test document
        await updateDoc(doc(db, 'tests', testId), {
        ...
        showSnackbar(`Failed to add question: ${e.message}`, 'error');
        console.error('Error adding question below:', e);
    } finally {
        // Reset the form and parent state
        clearQuestionForm();
        setParentQuestionForInsert(null);
    }
};
...
    // Set the state for the question being edited
    setEditingQuestionId(question.id);
    setEditingQuestionIndex(questions.findIndex(q => q.id === question.id));
    
...
    setQuestionSection(question.sectionName ?? '');
...
       {/* NEW: Section Management Section */}
{/* NEW: Section Management Section */}
<div className="p-8 bg-gray-50 rounded-xl shadow-lg">
  <h2 className="text-2xl font-bold text-gray-800 mb-6 border-b pb-4">Sections</h2>
  <div className="grid grid-cols-1 gap-4">
    {testSections.map((section, index) => (
      <div key={index} className="flex items-center justify-between p-2 rounded-lg hover:bg-gray-200 transition-colors bg-white">
        <div className="flex-1">
          {editingSectionName === section.name ? (
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter') {
                  renameSection(section.name, newSectionName);
                }
              }}
              className="w-full px-2 py-1 rounded-md border border-gray-300 focus:ring-2 focus:ring-red-500"
            />
          ) : (
            <span className="text-lg font-medium text-gray-800">{section.name}</span>
          )}
        </div>
        <div className="flex items-center gap-2 ml-4">
          {editingSectionName === section.name ? (
            <button onClick={() => renameSection(section.name, newSectionName)} className="p-2 text-green-600 hover:bg-green-200 rounded-lg transition-colors"><SaveIcon /></button>
          ) : (
            <button onClick={() => startRenameSection(section.name)} className="p-2 text-blue-600 hover:bg-blue-200 rounded-lg transition-colors"><EditIcon /></button>
          )}
          <button onClick={() => deleteSection(section.name)} className="p-2 text-red-600 hover:bg-red-200 rounded-lg transition-colors"><DeleteIcon /></button>
        </div>
      </div>
    ))}
  </div>
  {/* Input and button to add a new section */}
  <div className="flex items-center gap-4 mt-4">
    ...
    <button
      onClick={() => {
        const inputElement = document.querySelector('input[placeholder="New Section Name"]');
        if (inputElement?.value.trim()) {
          addSection(inputElement.value);
          inputElement.value = '';
        }
      }}
      className="py-3 px-6 rounded-lg bg-green-600 text-white font-bold hover:bg-green-700 transition-colors duration-200"
    >
      Add
    </button>
  </div>
...
<div className="mb-4">
  ...
  <select
    ...
    className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-red-500"
  >
    {/* MODIFIED: Add a 'None' option with a null value */}
  ...
  <div className="border p-4 rounded-lg bg-gray-50">
    ...
    {/* Question Text Input (Part 2) - Visible only for 'inBetween' placement */}
    {isImageInBetween && (
...
{/* NEW: Conditional UI feedback message */}
  {parentQuestionForInsert && (
     <div className="text-lg text-green-700 p-4 bg-green-50 border-l-7 border-green-400 mb-4 rounded-r-lg">
      <p>Adding a new question below Question No. {parentQuestionForInsert.order}</p>
    </div>
  )}
    {/* Action Buttons */}
<div className="mt-8 flex justify-between gap-4">
  <button
    onClick={parentQuestionForInsert ? saveQuestionBelow : addOrUpdateQuestion}
    className="w-full py-4 px-6 rounded-lg bg-red-700 text-white font-bold shadow-md transition-all duration-200 hover:bg-red-800 hover:shadow-xl active:scale-99"
  >
    {editingQuestionId
      ? 'Update Question'
      : parentQuestionForInsert
  ...
  {(editingQuestionId || parentQuestionForInsert) && (
...
<div className="p-8 bg-white rounded-xl shadow-lg flex-1 flex flex-col">
  <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 border-b pb-4">
    <h2 className="text-2xl font-bold text-gray-800">Existing Questions</h2>
    <div className="flex items-center gap-4 mt-4 md:mt-0">
      {/* NEW: Question Count Badge */}
      ...
      >
        ...
        {testSections.map((section, index) => {
          const sectionQuestionCount = questions.filter(q => q.sectionName === section.name).length;
          return (
            <option key={index} value={section.name}>
              {section.name} ({sectionQuestionCount})
            </option>
          );
        })}
      </select>
            ...
            <ul {...provided.droppableProps} ref={provided.innerRef} className="space-y-6">
               {filteredQuestions.map((question, index) => (
                <React.Fragment key={question.id}>
                  {/* Conditionally render the in-line edit form */}
                  {editingQuestionId === question.id ? (
                    <li className="p-4 bg-yellow-100 rounded-xl shadow-inner border-2 border-yellow-400">
                      <h3 className="text-xl font-bold text-gray-800 mb-4">Edit Question {index + 1}</h3>
                      <div className="border p-4 rounded-lg bg-white">
                          {/* NEW: Section Selector Dropdown for Edit Form */}
                        <div className="mb-4">
                          <label className="block text-sm font-semibold text-gray-700 mb-2">Select Section</label>
                          <select
                            value={questionSection}
...
                        </div>
                        ...
                        <div className="mt-6 border-t pt-4">
                            <h4 className="text-lg font-bold text-gray-800 mb-4">Options</h4>
                            {options.slice(0, enableOptionE ? 5 : 4).map((option, optIndex) => (
                                <div key={optIndex} className="flex items-start gap-4 mb-4">
                                    <div className="flex-shrink-0 flex flex-col items-center gap-2 mt-1">
                                        <input type="checkbox" checked={option.isCorrect} onChange={() => setOptions(opts => opts.map((o, i) => i === optIndex ? { ...o, isCorrect: !o.isCorrect } : { ...o, isCorrect: false }))} className="form-checkbox h-5 w-5 text-green-600 rounded-full flex-shrink-0" />
                                        <label className="text-xs font-bold text-gray-800">{String.fromCharCode(65 + optIndex)}</label>
                                    </div>
                                    <div className="flex-1 space-y-2">
                                        <div className="flex items-start gap-2">
                                            <textarea placeholder={`Option ${String.fromCharCode(65 + optIndex)}`} value={option.text} onChange={(e) => setOptions(opts => opts.map((o, i) => i === optIndex ? { ...o, text: e.target.value } : o))} onPaste={(e) => handleTextPaste(e, true, optIndex)} className="flex-1 px-4 py-2 rounded-lg border border-gray-300 focus:ring-2 focus:ring-red-500" rows="1" />
                                            <div className="flex flex-col gap-1 items-center">
                                                <label className="text-xs text-gray-700 flex items-center cursor-pointer">
                                                    LaTeX
                                                    <input type="checkbox" className="form-checkbox ml-2 rounded" checked={option.isLatexOption} onChange={(e) => setOptions(opts => opts.map((o, i) => i === optIndex ? { ...o, isLatexOption: e.target.checked } : o))} />
                                                </label>
                                                <button onClick={() => document.getElementById(`option-image-upload-${optIndex}`).click()} className="p-1 rounded-lg bg-gray-200 hover:bg-gray-300 text-gray-700 transition-colors">
                                                    <ImagePlusIcon className="h-4 w-4" />
                                                </button>
                                                <input type="file" id={`option-image-upload-${optIndex}`} className="hidden" accept="image/*" onChange={(e) => handleFileChange(e, true, optIndex)} />
                                            </div>
                                        </div>
                                        {isUploadingOptionImage[optIndex] && <div className="w-full bg-gray-200 rounded-full h-2.5"><div className="bg-red-600 h-2.5 rounded-full animate-pulse"></div></div>}
                                        {option.imageUrl && (
                                            <div className="relative p-2 bg-gray-100 rounded-lg">
                                                <img src={option.imageUrl} alt={`Option ${optIndex}`} className="max-h-24 object-contain mx-auto" />
                                                <button onClick={() => clearImage(true, optIndex)} className="absolute top-1 right-1 bg-white rounded-full p-1 shadow">
                                                    <XCircleIcon className="text-red-500 h-5 w-5" />
                                                </button>
                                            </div>
                                        )}
                                        {option.isLatexOption && (
                                            <LatexToolbar onAddLatex={(latex) => setOptions(opts => opts.map((o, i) => i === optIndex ? { ...o, text: o.text + latex } : o))} />
                                        )}
                                    </div>
                                </div>
                            ))}
                    ...
                    // Render the draggable question display item
  ...
  {/* NEW: The "Add Question Below" button */}
  <button onClick={() => handleAddQuestionBelow(question)} className="p-2 bg-green-100 rounded-lg hover:bg-green-200 transition-colors">
    <ArrowDownCircleIcon className="text-green-600" />
  </button>
</div>
                        </li>
                      )}
